...
const DISPLAY_NAME = "Azure Functions Hosted Skills (preview-11)";
...
const OWNERSHIP_DIRECTORY = ".azure-functions-hosted-skills-preview-11";
const OWNERSHIP_FILE = "ownership.json";
const MODEL_FILE = "model.json";
const LOCAL_SETTINGS_FILE = "local.settings.json";
const TIMER_AGENT_FILE = path.join("src", "daily-repo-digest.agent.md");
const localHosts = new Map();
const azuriteInstallPromises = new Map();
let localRuntimeStartupTail = Promise.resolve();
...
const MODEL_PREFERENCE_FILE = path.join(WORKSPACE_ROOT, OWNERSHIP_DIRECTORY, "model-selection.json");
...
async function availableLocalHostPort(preferred = 7071) {
	for (let port = preferred; port < preferred + 20; port += 1) {
		if (!(await tcpPortListening(port))) return port;
	}
	throw new Error(`No available local function host port was found from ${preferred} through ${preferred + 19}.`);
}
...
async function withLocalRuntimeStartupLock(callback) {
	const previous = localRuntimeStartupTail;
	let release;
	localRuntimeStartupTail = new Promise((resolve) => { release = resolve; });
	await previous;
	try {
...
async function startLocalHostedSkillUnlocked(input) {
	assertConfirmed(input);
	const workspacePath = await assertOwnedWorkspace(input.workspacePath);
	const requestedPort = validPort(input.port);
	const port = input.preferAvailablePort ? await availableLocalHostPort(requestedPort) : requestedPort;
...
async function startLocalHostedSkill(input) {
	return withLocalRuntimeStartupLock(() => startLocalHostedSkillUnlocked(input));
}
...
async function azJson(args) {
	...
	if (TEST_MODE && args[0] === "resource" && args[1] === "list") {
		...
		const rows = process.env.FUNCTION_CANVAS_TEST_FIRST_EMPTY_MODEL === "1" ? [{
			id: `/subscriptions/${subscription}/resourceGroups/fixture-rg/providers/Microsoft.CognitiveServices/accounts/empty-foundry/projects/empty-project`,
			name: "empty-project",
		}] : [];
		rows.push({
		...
		if (process.env.FUNCTION_CANVAS_TEST_MULTIPLE_MODELS === "1") rows.push({
			id: `/subscriptions/${subscription}/resourceGroups/fixture-rg/providers/Microsoft.CognitiveServices/accounts/fixture-foundry/projects/preferred-project`,
			name: "preferred-project",
		});
		return rows;
	}
	if (TEST_MODE && args[0] === "cognitiveservices" && args[1] === "account" && args.includes("show")) {
		const accountName = String(args[args.indexOf("--name") + 1] || "fixture-foundry");
		return { properties: { endpoints: { "AI Foundry API": `https://${accountName}.invalid` } } };
	}
	if (TEST_MODE && args[0] === "cognitiveservices" && args[1] === "account" && args[2] === "deployment" && args.includes("list")) {
		const accountName = String(args[args.indexOf("--name") + 1] || "fixture-foundry");
		if (accountName === "empty-foundry") return [];
...
function publicState(instanceId) {
	return {
		target: "local",
		trigger: "timer",
		triggerTypes: [{ id: "timer", label: "Timer" }],
		hero: { title: "Hosted Skill" },
		prompt: "",
...
function parseAgentPresentation(source) {
	const normalized = String(source || "").replace(/\r\n/g, "\n");
	const match = normalized.match(/^---\n([\s\S]*?)\n---\n?([\s\S]*)$/);
	if (!match) throw new Error("The generated Timer agent file has no valid front matter.");
	const name = match[1].match(/^name:\s*(.+)$/m)?.[1]?.trim() || "Hosted Skill";
	const triggerType = match[1].match(/^\s*type:\s*(.+)$/m)?.[1]?.trim() || "";
	const prompt = match[2].trim();
	if (!prompt) throw new Error("The generated Timer agent file has no instructions.");
	return {
		hero: { title: name },
		trigger: triggerType === "timer_trigger" ? "timer" : triggerType || "timer",
		prompt,
	};
}

async function hydrateSkillPresentation(entry) {
	if (!entry.workspacePath) return;
	const source = await readFile(path.join(entry.workspacePath, TIMER_AGENT_FILE), "utf8");
	const presentation = parseAgentPresentation(source);
	entry.state.hero = presentation.hero;
	entry.state.trigger = presentation.trigger;
	entry.state.prompt = presentation.prompt;
}

async function readModelPreference() {
	try {
		const preference = JSON.parse(await readFile(MODEL_PREFERENCE_FILE, "utf8"));
		return preference?.provider === "foundry" && preference.endpoint && preference.model
			? preference
			: null;
	} catch (error) {
...
async function writeModelPreference(metadata) {
	await mkdir(path.dirname(MODEL_PREFERENCE_FILE), { recursive: true });
	await writeFile(MODEL_PREFERENCE_FILE, `${JSON.stringify({
		provider: "foundry",
...
async function refreshFoundryModels(entry, generation = ++entry.modelGeneration) {
	...
	if (!foundry.some((item) => item.id === entry.state.modelBinding.resourceId)) {
		const firstUsable = foundry.find((item) => item.models.length) || foundry[0];
		entry.state.modelBinding.resourceId = firstUsable?.id || "";
		entry.state.modelBinding.modelId = firstUsable?.models[0]?.id || "";
	...
	const preferred = entry.persistedModel || entry.preferredModel;
	if (preferred?.provider === "foundry") {
		const persistedResource = foundry.find((item) => entry.modelResources.get(item.id)?.endpoint === preferred.endpoint);
		const persistedModel = persistedResource?.models.find((item) => item.id === preferred.model);
		if (persistedResource && persistedModel) {
			...
			if (entry.persistedModel) {
				...
				entry.state.modelBinding.readiness = { state: "ready", message: entry.state.modelBinding.status };
			}
		}
	}
	entry.persistedModel = null;
	entry.preferredModel = null;
	return true;
}
...
async function initializePublicCanvas(entry) {
	...
	entry.initializePromise = (async () => {
		...
		if (entry.workspacePath) {
			try {
				await hydrateSkillPresentation(entry);
			} catch (error) {
				entry.state.sourceWorkspace.error = `Could not load the generated skill instructions: ${shortError(error)}`;
			}
		}
		...
		try {
			...
			if (createdWorkspace) entry.preferredModel = await readModelPreference();
			await refreshFoundryModels(entry);
			autoApplyModel = Boolean(
				entry.workspacePath &&
				entry.state.modelBinding.resourceId &&
				entry.state.modelBinding.modelId &&
				(createdWorkspace || entry.state.modelBinding.configured),
			);
		} catch (error) {
		...
		if (autoApplyModel) await applySelectedFoundryModel(entry);
...
async function applySelectedFoundryModel(entry) {
	...
	binding.readiness = { state: "ready", message: binding.status };
...
function handleUiRequest(entry, request, response) {
	...
	const routes = new Map([
		...
		["/invoke", async (body) => {
			...
			const result = await invokeLocalHostedSkill({
				workspacePath: entry.workspacePath,
				prompt: String(body.prompt || "").trim() || entry.state.prompt,
				confirm: true,
			});
...
const canvas = createCanvas({
	id: "azure-functions-hosted-skills-preview-11",
