...
const LEGACY_DAILY_DIGEST_AGENT_SIGNATURES = new Map([
...
async function migrateOwnedLegacyDailyDigestAgents(root, manifest) {
	const currentFiles = [];
	const baseline = new Map((manifest?.baseline || []).map((item) => [path.normalize(item.path), item]));
	for (const [relativePath, legacySignature] of LEGACY_DAILY_DIGEST_AGENT_SIGNATURES) {
		...
		if (
			normalizedTextDigest(current) !== legacySignature ||
			owned?.type !== "file" ||
			owned.size !== currentBytes.byteLength ||
			owned.sha256 !== createHash("sha256").update(currentBytes).digest("hex")
		) {
	...
	for (const item of currentFiles) {
		const content = await readFile(path.join(BUNDLED_TEMPLATE_DIRECTORY, item.normalizedPath), "utf8");
	...
	return {
		...
		manifest: {
			...manifest,
			baseline: manifest.baseline.map((item) => replacements.get(path.normalize(item.path)) || item),
		},
	};
}
