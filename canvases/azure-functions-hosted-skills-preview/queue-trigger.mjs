...
export async function enqueueLocalQueueMessage({
	...
	if (typeof runAzureCliText !== "function") throw new TypeError("runAzureCliText is required.");
	...
	const options = { timeout: 30000, maxBuffer: 1024 * 1024 };
	await runAzureCliText(["storage", "queue", "create", "--name", queueName, ...common, "-o", "none"], options);
	await runAzureCliText(
