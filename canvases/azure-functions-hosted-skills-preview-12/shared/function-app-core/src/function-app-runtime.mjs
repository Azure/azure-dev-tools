...
import { buildHttpPostRequest } from "./http-request.mjs";
...
function httpRequest(fn, input, key, draft) {
	...
	if (method === "GET") {
		...
		return {
			...
			init: {
				method: "GET",
				headers: usesKey ? { "x-functions-key": key } : {},
				body: undefined,
			},
			display: {
				method: "GET",
				url: url.toString(),
				auth: usesKey ? "function key [redacted]" : "anonymous",
				headers: usesKey ? { "x-functions-key": "[redacted]" } : {},
				body: "",
				overriddenHeaders: [],
			},
		};
	...
	const request = buildHttpPostRequest(
		...
		},
		{
			...
			authorizationLabel: !usesKey
				? "anonymous"
				: cleanString(fn.authLevel).toLowerCase() === "admin"
					? "master key [redacted]"
					: "function key [redacted]",
		},
	);
	return {
		url: url.toString(),
