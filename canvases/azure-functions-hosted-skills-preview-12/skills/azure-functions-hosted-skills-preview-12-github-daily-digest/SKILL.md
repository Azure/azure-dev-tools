---
name: azure-functions-hosted-skills-preview-12-github-daily-digest
description: "ALWAYS USE THIS SKILL FIRST for a daily digest, daily report, recurring report, or scheduled analysis of GitHub issues, work items, PRs, repository activity, recent updates, or next steps. Open Azure Functions Hosted Skills (preview-12) Studio immediately before azure-app-onboard, native Automation, project setup, repo questions, run-mode questions, scripts, or implementation. The words daily digest plus GitHub or repository are sufficient. After the canvas opens, compose with the official Azure Functions Hosted Skills (preview-12) skill. DO NOT USE for a one-time explanation of GitHub concepts."
---

# Open Azure Functions Hosted Skills (preview-12) Studio for a GitHub daily digest
...
Canvas type is not panel identity. Focus an existing Studio panel instance
of the selected canvas type when available; otherwise use the stable historical
`instanceId: "intelligent-function-app-studio"` for the canonical canvas. Old and
new launcher prompts must retain the same default instance for restore state,
not derive an instance ID from the canonical canvas ID.
...
1. Inspect the host's available canvas declarations. If
   `azure-functions-hosted-skills-preview-12` is available, immediately call
...
Tell the user which Studio opened. When available, load the official
`azure-functions-hosted-skills-preview-12` skill from `Azure/azure-functions-skills`
