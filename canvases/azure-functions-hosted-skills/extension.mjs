...
function installAgentResponseLogging(source) {
  ...
  const cleanSource = input.replace(ORIGINAL_AGENT_RESPONSE_LOGGING, "").replace(VERSION_TWO_AGENT_RESPONSE_LOGGING, "").replace(VERSION_THREE_OR_FOUR_AGENT_RESPONSE_LOGGING, "").replace(VERSION_FIVE_AGENT_RESPONSE_LOGGING, "").replace(LEGACY_AGENT_RESPONSE_LOGGING, "");
  const loggerSetup = [
    "import logging",
    "import json",
    "",
...
var LEGACY_GATEWAY_PROVIDER_MARKER = "# Intelligent Function App Studio: AI Gateway provider";
var LEGACY_GATEWAY_PROVIDER_V2_MARKER = "# Intelligent Function App Studio: model provider routing v2";
var GATEWAY_PROVIDER_MARKER = "# Azure Functions Hosted Skills: model provider routing v2";
var LEGACY_GATEWAY_PROVIDER_BLOCK = `${LEGACY_GATEWAY_PROVIDER_MARKER}
...
async function ensureGatewayProviderFilesUnlocked(entry, mcpMode) {
  ...
  const providerBlock = `${GATEWAY_PROVIDER_MARKER}
...
`;
  ...
  if (current.includes(LEGACY_GATEWAY_PROVIDER_V2_MARKER)) {
    ...
    next = current.replace(importLine, `import os

from azure_functions_agents import create_function_app, set_client_manager`).replace(appLine, `${providerBlock}
${appLine}`);
  }
