...
const DISPLAY_NAME = "Azure Functions Hosted Skills Preview 3";
...
const OWNERSHIP_DIRECTORY = ".azure-functions-hosted-skills-preview-3";
const OWNERSHIP_FILE = "ownership.json";
const MODEL_FILE = "model.json";
const LOCAL_SETTINGS_FILE = "local.settings.json";
const localHosts = new Map();
const TEST_MODE = process.env.FUNCTION_STUDIO_TEST_MODE === "1";
...
async function azJson(args) {
	if (TEST_MODE && args[0] === "resource" && args[1] === "list") {
		return [{
			...
			properties: {
				endpoint: "https://fixture-foundry.invalid",
...
function publicRuntimeClient() {
  ...
  $('model-resource')?.addEventListener('change', () => post('/models/select-choice', { resourceId: $('model-resource').value, modelId: $('model-model')?.value || '' }));
...
function publicState() {
	return {
		target: "local",
		sourceWorkspace: { mode: "current", relativePath: "hosted-skill", materialized: false, operation: "", error: "", canUseCurrent: true, autoCreate: true },
		modelBinding: { source: "foundry", subscription: "", foundry: [], gateways: [], resourceId: "", modelId: "", configured: false, loading: false, status: "", error: "", readiness: { state: "select", message: "Create a local workspace, then select a Microsoft Foundry model." } },
		doctor: null,
		doctorRunning: false,
...
async function refreshFoundryModels(entry) {
	const rows = await azJson(["resource", "list", "--resource-type", "Microsoft.CognitiveServices/accounts/projects"]);
	entry.modelResources.clear();
	entry.state.modelBinding.foundry = rows
		.map((item) => {
			const id = String(item.id || "");
			const endpoint = String(item.properties?.endpoint || "");
			const models = Array.isArray(item.properties?.models) ? item.properties.models
				.map((model) => ({ id: String(model.name || model.id || ""), label: String(model.name || model.id || "") }))
				.filter((model) => model.id) : [];
			if (id && endpoint) entry.modelResources.set(id, { endpoint, models });
			return {
			id: String(item.id || ""),
		...
		.filter((item) => item.id && entry.modelResources.has(item.id));
...
const canvas = createCanvas({
	id: "azure-functions-hosted-skills-preview-3",
