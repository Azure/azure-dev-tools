...
const DISPLAY_NAME = "Azure Functions Hosted Skills Preview 2";
...
const OWNERSHIP_DIRECTORY = ".azure-functions-hosted-skills-preview-2";
...
</style></head><body><main><h1>${DISPLAY_NAME}</h1><p class="lead">Create a local Microsoft Foundry hosted skill, run it locally, and inspect function output and its response.</p>
<div class="card"><div class="row"><button id="doctor" type="button">Doctor</button><button id="subscriptions" type="button">List subscriptions</button></div><div id="status" class="notice" role="status">Ready.</div><pre id="result" hidden></pre></div>
<div class="card"><h2>Local workspace</h2><div class="grid"><label>New workspace path<input id="workspace-path" autocomplete="off"></label><label>Foundry endpoint<input id="foundry-endpoint" autocomplete="off"></label><label>Model deployment<input id="foundry-model" autocomplete="off"></label><label>Managed identity client ID (optional)<input id="managed-identity-client-id" autocomplete="off"></label></div><div class="row"><button id="create" type="button">Create workspace</button><button id="configure" type="button">Configure model</button><button id="edit-instructions" type="button">Edit instructions</button><button id="open-vscode" type="button">Open in VS Code</button></div></div>
<div class="card"><h2>Local invocation</h2><div class="grid"><label>Port<input id="local-port" type="number" value="7071" min="1024" max="65535"></label><label>Prompt<textarea id="prompt">Describe the current task in one sentence.</textarea></label></div><div class="row"><button id="start-local" type="button">Start local function</button><button id="stop-local" type="button">Stop local function</button><button id="invoke-local" type="button">Invoke</button><button id="prepare-deployment" type="button">Prepare deployment</button></div><h2>Command output</h2><pre id="command-output">No local host output yet.</pre><h2>Agent response</h2><pre id="agent-response">No invocation response yet.</pre></div>
<script>const $=id=>document.getElementById(id);function setStatus(message,ok){const el=$("status");el.textContent=message||"Ready.";el.className="notice "+(ok===false?"error":ok===true?"success":"")}function value(id){return $(id)?.value?.trim()||""}async function post(route,body={}){setStatus("Working...");try{const response=await fetch(route,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)}),result=await response.json();if(!result.ok)throw new Error(result.message||"Request failed.");setStatus(result.message||"Completed.",true);return result}catch(error){setStatus(error.message||String(error),false);throw error}}const input=()=>({workspacePath:value("workspace-path")});$("doctor").onclick=async()=>{try{const result=await post("/doctor");$("result").hidden=false;$("result").textContent=JSON.stringify(result.checks,null,2)}catch{}};$("subscriptions").onclick=async()=>{try{const result=await post("/subscriptions");$("result").hidden=false;$("result").textContent=JSON.stringify(result.subscriptions,null,2)}catch{}};$("create").onclick=async()=>{try{await post("/workspace/create",input())}catch{}};$("configure").onclick=async()=>{try{await post("/workspace/configure",{...input(),provider:"foundry",endpoint:value("foundry-endpoint"),model:value("foundry-model"),managedIdentityClientId:value("managed-identity-client-id")})}catch{}};$("edit-instructions").onclick=async()=>{try{await post("/workspace/edit-instructions",input())}catch{}};$("open-vscode").onclick=async()=>{try{await post("/workspace/open-vscode",input())}catch{}};$("start-local").onclick=async()=>{try{const result=await post("/local/start",{...input(),port:Number(value("local-port"))});$("command-output").textContent=result.output||"Local host started."}catch{}};$("stop-local").onclick=async()=>{try{await post("/local/stop",input())}catch{}};$("invoke-local").onclick=async()=>{try{const result=await post("/local/invoke",{workspacePath:value("workspace-path"),prompt:value("prompt")});$("command-output").textContent=result.commandOutput||"";$("agent-response").textContent=result.response||""}catch{}};$("prepare-deployment").onclick=async()=>{try{const result=await post("/deployment/prepare",input());$("command-output").textContent=result.message||""}catch{}};</script></main></body></html>`;
}
...
function handleUiRequest(request, response) {
	...
	const routes = new Map([
		...
		["/workspace/edit-instructions", editHostedSkillInstructions],
...
async function openPage(instanceId) {
	...
	if (!server) {
		server = createServer((request, response) => handleUiRequest(request, response));
...
const canvas = createCanvas({
	id: "azure-functions-hosted-skills-preview-2",
